...
public class AppointmentRepository {
    ...
    public List<Appointment> findManyByDoctor(Doctor doctor) {
        TypedQuery<Appointment> query =
                em.createQuery("FROM Appointment a WHERE a.doctor = ?1", Appointment.class);
    ...
    public List<Appointment> findManyByPatient(Patient patient) {
        TypedQuery<Appointment> query =
                em.createQuery("FROM Appointment a WHERE a.patient = ?1", Appointment.class);
    ...
    public List<Appointment> findManyByTimestamp(Timestamp timestamp) {
        TypedQuery<Appointment> query =
                em.createQuery("FROM Appointment a WHERE a.timestamp = ?1", Appointment.class);
    ...
    public List<Appointment> findManyByStatus(AppointmentStatus status) {
        TypedQuery<Appointment> query =
                em.createQuery("FROM Appointment a WHERE a.status = ?1", Appointment.class);
